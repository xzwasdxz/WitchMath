...
int main() {
	...
	size_t max_amount = 87;
	double tau = 2.5;
	size_t n_x_steps = 10000;
	size_t n_t_steps = 10000;
	double t_step = 1/n_t_steps;
	double x_step = 1/n_x_steps;
	double* sqrt_lambda = new double[max_amount];
	sqrt_lambda = roots(max_amount, 1, sqrt_lambda); // searching lambdas
	double u = 0;
	double* utau2 = new double[n_x_steps]; // plot for t=2*tau assuming l=1 
	thread t1 (it1, n_x_steps, x_step, sqrt_lambda, max_amount, utau2, tau);
	double* utau5 = new double[n_x_steps];
	thread t2 (it2, n_x_steps, x_step, sqrt_lambda, max_amount, utau5, tau);// plot for t=5*tau assuming l=1 
	double* ul3 = new double[5 * n_t_steps * tau];
	thread t3 (it3, n_t_steps, t_step, sqrt_lambda, max_amount, ul3, tau);// plot for x=l/3 assuming l=1 
	t1.join();
	t2.join();
	t3.join();
